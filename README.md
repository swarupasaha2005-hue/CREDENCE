...
|---|---|---|
| `ci.yml` | Every push and pull request, any branch | Builds the SDK bindings, lints and builds the frontend (`next build`), runs frontend tests if defined, and for every contract under `contracts/*` runs `cargo check`, `cargo test`, and a `cargo build --release --target wasm32v1-none`. See [CI](#️-ci) below for the full breakdown. |
...
**Smart contracts have no GitHub Actions deployment workflow, intentionally.** Contract deployment is a manual, one-time-per-network operation run locally via the `stellar contract deploy` CLI (see [Deployment](#️-deployment) above), with the resulting addresses committed to `registry/deployments.json`. Automating this on every push would risk silently redeploying (and changing the address of) a live Testnet contract that other deployed frontends and recorded transactions already point to — CI only ever *tests and builds* the contracts (`cargo check` / `cargo test` / `cargo build --release --target wasm32v1-none`), it never deploys them.
...
| `frontend` | Installs dependencies, builds the SDK bindings under `packages/sdk/bindings/*`, runs `npm run lint` (ESLint), runs `npm run build` (`next build`), and runs the frontend test script if one is defined in `package.json`. | Yes |
| `contracts` | For every contract under `contracts/*`: runs `cargo check`, then `cargo test`, then `cargo build --release --target wasm32v1-none` to verify the deployable WASM artifact actually compiles. | Yes |

Generated Stellar contract bindings under `packages/sdk/bindings/**` are excluded from ESLint (`eslint.config.mjs`) — they're machine-generated by the Stellar CLI's binding generator, not hand-written application code.
