    ...
    <AnimatePresence>
      {open && position && (
        <motion.div
          ...
          onClick={onClose}
        >
          <RepayModalContent key={position.symbol} position={position} onClose={onClose} />
        </motion.div>
      )}
    </AnimatePresence>
  );
...
interface RepayModalContentProps {
...
function RepayModalContent({ position, onClose }: RepayModalContentProps) {
  ...
  const outstandingQuantity = Number(position.borrowedAmount) / 10 ** position.decimals;
  const exceedsMax = isValidAmount && parsedAmount > outstandingQuantity;
  const remainingDebtQuantity = isValidAmount ? Math.max(0, outstandingQuantity - parsedAmount) : outstandingQuantity;
  ...
  const handleConfirm = async () => {
    ...
    const amountRaw = BigInt(Math.round(parsedAmount * 10 ** position.decimals));
    try {
      await mutation.mutateAsync({ symbol: position.symbol, amount: amountRaw, signer: address });
      toast.success("Repayment successful");
      trackProtocolAction("repay", { symbol: position.symbol, walletType: walletId });
    } catch (error) {
  ...
  return (
    <motion.div
      role="dialog"
      aria-modal="true"
      aria-label={`Repay ${position.symbol}`}
    ...
    >
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-medium text-market-text">Repay {position.symbol}</h2>
        </div>
        <button
          onClick={onClose}
          aria-label="Close"
      ...
      <dl className="mb-4 grid grid-cols-2 gap-y-2 text-sm">
        <dt className="text-market-text-muted">Outstanding Balance</dt>
      ...
      <div className="mb-2 flex items-center justify-between text-sm">
        <label htmlFor="repay-modal-amount" className="text-market-text-secondary">
          Amount
        </label>
        <span className="text-market-text-muted">
          Outstanding: {formatTokenQuantity(position.borrowedAmount, position.decimals)} {position.symbol}
        </span>
      </div>
      <div className="mb-4 flex items-center gap-2 rounded-xl border border-market-border bg-market-surface-hover px-4 py-3">
        <input
          id="repay-modal-amount"
          type="number"
          min="0"
          step="any"
          inputMode="decimal"
      ...
      {exceedsMax && <p className="mb-4 text-xs text-market-text-secondary">Amount exceeds outstanding balance.</p>}

      <dl className="mb-6 space-y-2 rounded-xl border border-market-border p-4 text-sm">
        <div className="flex items-center justify-between">
          <dt className="text-market-text-muted">Remaining Debt</dt>
          <dd className="tabular-nums text-market-text">
            {remainingDebtQuantity.toLocaleString()} {position.symbol}
          </dd>
        </div>
      </dl>
      ...
      >
        {isSubmitting ? "Submitting..." : "Confirm Repay"}
      </button>
    </motion.div>
  );
}
